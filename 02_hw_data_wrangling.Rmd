...
incumbency_table <- fb_ads %>% group_by(incumbency) %>% summarize(n=n())
kable(incumbency_table)
```
There are 7014 rows/candidates in the dataset.
...
knitr::kable(party_incumbent_promote, col.names = c("Party", "Incumbency", "Proportion of Ads Promoting"))
```
Democratic incumbents have the most promoting ads on average.
...
```{r}
fb_ads = fb_ads %>% mutate(impressions_millions= impressions / 1e6)

plot_q3 <- fb_ads %>% filter(office == "US House") %>% 
  ggplot(mapping=aes(x=impressions_millions)) +
  geom_histogram()

plot_q3
```
The plot is highly skewed right; few House candidates had more than 10 million ad impressions on Facebook.
...
fb_top_plot <- fb_ads %>% 
  ...
  head(15) %>% 
  ggplot(mapping=aes(y=reorder(cand_name_last, impressions_millions), x=impressions_millions, fill=party)) +
  geom_col() +
  scale_fill_manual(values=c("R"="red", "D"="blue")) +
  labs(title="Top 15 House Candidate Facebook Ad Impressions, 2017-2018", x="Facebook Ad Impressions (in millions)", y="Candidate Last Name") +
  theme_minimal()
